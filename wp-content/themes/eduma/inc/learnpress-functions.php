	...
	function thim_learnpress_breadcrumb() {
		...
		if ( is_single() ) {
			...
			if ( get_post_type() == 'lpr_course' ) {
				// all projects
				echo '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a itemprop="item" href="' . esc_url( get_post_type_archive_link( 'lpr_course' ) ) . '" title="' . esc_attr__( 'all projects', 'eduma' ) . '"><span itemprop="name">' . esc_html__( 'all projects', 'eduma' ) . '</span></a></li>';
			} else {
	...
	function thim_learnpress_page_title() {
		if ( get_post_type() == 'lpr_course' ) {
			if ( is_tax() ) {
				...
				echo esc_html__( 'all projects', 'eduma' );
			}
		}
