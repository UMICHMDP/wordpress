							...
							<div class="price-wrap">
								<?php if ( pmpro_isLevelFree( $level ) ): ?>
									...
									if ( $level->expiration_number ) {
										echo '<p class="expired">' . sprintf( __( "per %d %s.", "eduma" ), $level->expiration_number, pmpro_translate_billing_period( $level->expiration_period, $level->expiration_number ) ) . '</p>';
									} else {
										echo '<p class="expired">' . esc_html( 'Unlimited', 'eduma' ) . '</p>';
									}
								...
								<?php else: ?>
									...
									$price_text      = '<p class="price">' . $cost_text . '</p>';

									echo ent2ncr( $price_text );
