...
function add_event_taxonomy_to_event(){

    //set the name of the taxonomy
    $taxonomy = 'event';
    //set the post types for the taxonomy
    $object_type = 'tp_event';
...
add_action('init','add_event_taxonomy_to_event');

function display_event_taxonomy_terms($post_id){
    
    //get all terms assigned to this post
    $event_terms = get_the_terms($post_id,'event'); 
    //if we have member terms assigned to this post
    if($event_terms){
        echo '<div class="event-terms-meta">';
        echo '<span class="term-title"> Event Terms: </span>';
        //loop through each term 
        foreach($event_terms as $term){
            //collect term information and display it
            $term_name = $term->name;
            $term_link = get_term_link($term,'event'); 
