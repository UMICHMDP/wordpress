...
function learn_press_certificate_upload_dir( $subfolder = false ) {
	...
	if ( empty( $upload_dir[$subfolder] ) ) {
		...
		$folder                 = 'certificates';
		$dir                    = $_upload_dir['basedir'] . "/{$folder}";
		$url                    = $_upload_dir['baseurl'] . "/{$folder}";
...
function learn_press_get_certificate_tab_slug() {
	return sanitize_title( __( 'certificates', 'learnpress-certificates' ) );
}
...
function learn_press_get_user_certificates( $user_id = 0, $course_id = 0 ) {
	...
	if ( $_results = $wpdb->get_results( $query ) ) {
		...
		foreach ( $_results as $res ) {
			...
			$course    = LP_Course::get_course( $res->ID );
			$cert_id   = $res->cert_id;
...
function learn_press_certificate_permalink( $cert_id, $user_id = 0 ) {
	$user = learn_press_get_user( $user_id ? $user_id : get_current_user_id() );
	$url  = add_query_arg( '', '', learn_press_get_page_link( 'profile' ) . $user->user_login ) . '/' . learn_press_get_certificate_tab_slug() . '?view=' . get_post_field( 'post_name', $cert_id );
...
function learn_press_certificates_locate_template( $name, $args = null ) {
	learn_press_locate_template( $name, $args, LP_ADDON_CERTIFICATES_THEME_PATH, LP_ADDON_CERTIFICATES_PATH . '/templates/' );
}
