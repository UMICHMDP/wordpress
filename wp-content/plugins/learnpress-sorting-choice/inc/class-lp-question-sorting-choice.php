	...
	class LP_Question_Sorting_Choice extends LP_Abstract_Question {
		...
		function reset_options( $quiz_id, $user_id ) {
			...
			if ( $quiz && $quiz->has( 'question', $this->id ) ) {
				$session = LP()->session;
				$session->remove( 'learn-press-question-sorting-choice-' . $session->id . '-' . $this->id );
			}
		}
		...
		function get_displaying_answers( $quiz_id, $user_id = null ) {
			$answered = $this->_get_user_question_answered( $user_id, $quiz_id );
		...
		function _get_user_question_answered( $user_id, $quiz_id ) {
			global $wpdb;
		...
		function render( $args = array() ) {
			...
			$tmpl     = LP_Addon_Question_Sorting_Choice::locate_template( 'question-content.php' );
			include $tmpl;
		}
